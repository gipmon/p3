...
public class VideoClube {
	...
	private Hashtable<Integer, Cliente> clientes = new Hashtable<Integer, Cliente>();
	...
	public int addUser(String nome, Data data_insc, int cc, int id, int nif){
		...
		clientes.put(Cliente.getNClientes(), value);
	...
	public int addUser(String nome, Data data_insc, int cc, int id, String curso){
		Estudante value = new Estudante(nome, data_insc, cc, id, curso);
		clientes.put(Cliente.getNClientes(), value);
		return Cliente.getNClientes();
	}
	...
	public String outEmprestimo(int id_filme, double rate){
		if(!filmeDisponivel(id_filme)){
			catalogo_de_videos.get(id_filme).setRating(rate);
	...
	public String removeUser(int id_user){
		...
		if(clientes.containsKey(id_user)){
			clientes.remove(id_user);
			exists = true;
		}
		if(exists){
			//devolução
	...
	public String userDetails(int id_user){
		if(clientes.containsKey(id_user)){
			return clientes.get(id_user).toString();
		}else if(clientes.containsKey(id_user)){
			return clientes.get(id_user).toString();
		}else{
	...
	public String listByRating(){
		String out = "";
		if(Rating.getNRatings()==0){
			out += "Não existem filmes com votação!";
		}else{
			TreeMap<Double, Integer> tm = new TreeMap<Double, Integer>();
			for(Entry<Integer, Video> entry : catalogo_de_videos.entrySet()){
				tm.put(entry.getValue().getRating().getMedio(), entry.getKey());
			}
			for(Entry<Double, Integer> entry : tm.descendingMap().entrySet()){
				out += "ID vídeo: "+entry.getValue()+"; Título: "+catalogo_de_videos.get(entry.getValue()).getTitulo()+"\n";
			}
		}
		return out;
	}
	...
	public String videoRating(int id_video){
		if(!catalogo_de_videos.containsKey(id_video)){
			return "O vídeo não existe!";
		}else if(catalogo_de_videos.get(id_video).getRating()==null){
			return "Não existe rating para o vídeo!";
		}else{
			Rating video = catalogo_de_videos.get(id_video).getRating();
