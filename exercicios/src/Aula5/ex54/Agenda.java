...
public class Agenda{
	...
	public void load(String name){
		...
		if(Format.fileOK(f)){
			for(Entry<String, Pessoa> set : Format.importData(f)){
				add(set.getKey(), set.getValue());
			}
		}
	}
	
	public void backup(String name, String tec){
		Format.backup(name, tec, pessoas(0));
	}
	...
	private String[] keys(int order){ // 0 => Normal, 1 => Ordenado por nome, 2=> ordenado por CC
		...
		if(order==1 || order==2){ 
			...
			if(order==1){
				for(String key : keys){
					tree.put(pessoas.get(key).getName(), key);
				}
			}else{
				for(String key : keys){
