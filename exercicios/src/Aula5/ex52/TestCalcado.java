...
public class TestCalcado {
	public static void main(String[] args) {
		...
		Desportivo calDesp[] = prope.getCalcadoDesportivo();
		System.out.println("Preço Total do Calçado Desportivo -> " + precoCalcado(calDesp));
		printFinalidade(calDesp); 
	}
	private static double precoCalcado(Desportivo cal[]){ 
		double sum = 0;
		for(Desportivo ca : cal){
			sum += ((Calcado)ca).getPreco();
		}
		return sum;
	}
	private static void printFinalidade(Desportivo cal[]){ 
		for(Desportivo ca : cal){
			System.out.println("\n"+ca);
		}
	}
